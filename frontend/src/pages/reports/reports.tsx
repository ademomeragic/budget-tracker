    ...
    const fetchData = async () => {
      try {
        const [txRes, walletRes, catRes] = await Promise.all([
          ...
          api.get("/wallet"),
          api.get("/category?type=both"),
        ]);

        console.log("Transactions fetched:", txRes.data);
...
const expenseByCategory = expenses.reduce((acc: any, curr) => {
  const categoryName =
    categories.find((c) => c.id === curr.categoryId)?.name || "Unknown";

  acc[categoryName] = (acc[categoryName] || 0) + getAmount(curr);
  return acc;
}, {});
